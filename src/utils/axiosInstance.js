...
/* =========================================================
   AXIOS INSTANCE (SAFE & NON-BLOCKING)
   ========================================================= */
...
const axiosInstance = axios.create({
  ...
  withCredentials: true,
  timeout: 8000, // ⛔ prevents infinite browser spinner
});
...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    if (!error.response) {
      ...
      error.userMessage = "Unable to reach server. Please try again later.";
      return Promise.reject(error);
    }
...
/* =========================================================
   CONTACT FORM (NO INTERCEPTORS, NO AUTH)
   ========================================================= */
