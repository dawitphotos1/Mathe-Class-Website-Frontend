...
const AdminDashboard = () => {
  ...
  /* ============================================================
     🔥 Backend Status Check (SIMPLIFIED - no warmup)
  ============================================================ */
  const checkBackendStatus = useCallback(async () => {
    try {
      await axiosInstance.get("/health", { timeout: 10000 });
  ...
  const manuallyWarmBackend = async () => {
    ...
    try {
      // Just check if backend is responsive
      await axiosInstance.get("/health", { timeout: 30000 });
      clearInterval(progressInterval);
      setWarmupProgress(100);
      toast.success("✅ Backend is responsive!");
      setBackendStatus("online");
    } catch (error) {
      clearInterval(progressInterval);
      setWarmupProgress(0);
      toast.error("❌ Backend is not responding. Please wait and try again.");
    }
  ...
  /* ============================================================
     ✅ APPROVE Student (SIMPLIFIED - no warmup)
  ============================================================ */
  const handleApproveStudent = async (id) => {
    setApprovingId(id);
    ...
    try {
      ...
      if (err.code === "ECONNABORTED" || err.message.includes("timeout")) {
        toast.error("⏰ Request timeout. Backend might be slow to respond.");
      } else {
  ...
  const handleApproveEnrollment = async (id) => {
    ...
    try {
      toast.info("🔄 Approving enrollment...");
      ...
      <div className={`backend-status ${backendStatus}`}>
        ...
        {backendStatus === "sleeping" && (
          <div className="warmup-section">
            ...
            <div className="status-note">
              ⚠️ Render free tier can take 30-60 seconds to wake up
            </div>
          </div>
        )}
      </div>

      {/* Rest of your AdminDashboard UI remains the same */}
      {/* ... (keep all the JSX from your original file, just remove the warmup references) */}
