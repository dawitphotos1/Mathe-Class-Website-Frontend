...
const AdminDashboard = () => {
  ...
  // 📌 Fetch pending students (matches backend `/pending-students`)
  const fetchPendingUsers = useCallback(async () => {
    setLoadingUsers(true);
    setErrorUsers("");
    try {
      const res = await axiosInstance.get("/admin/pending-students");
      setPendingUsers(res.data || []); // backend returns array directly
    } catch (err) {
      handleError(err, setErrorUsers);
    } finally {
      setLoadingUsers(false);
  ...
  // 📌 Approve / reject students (match backend `/approve/:id` and `/reject/:id`)
  const handleApproveUser = async (userId) => {
    try {
