  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (['admin', 'teacher'].includes(user.role)) {
        ...
        setPendingEnrollments(
          Array.isArray(pendingEnrollmentsRes.data) ? pendingEnrollmentsRes.data : []
        );
        setApprovedEnrollments(
          Array.isArray(approvedEnrollmentsRes.data) ? approvedEnrollmentsRes.data : []
        );
      }
    } catch (err) {
      handleError(err, () => {});
    ...
    <div className={`min-h-screen bg-light text-dark ${darkMode ? 'dark-mode' : ''}`}>
      <div className="container mx-auto p-4">
        <div className="bg-white shadow-lg rounded-lg p-6">
                ...
                <tbody>
                  {filteredPendingUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-50">
                      ...
                      <td className="border p-2">{u.subject}</td>
          ...
          {user.role === 'admin' && activeTab === 'approvedUsers' && (
            <>
              <h3 className="text-xl font-semibold mb-4">Total Approved Students</h3>
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 mb-4">
                <button
                  className="btn btn-secondary"
                  onClick={exportApprovedUsersToCSV}
          ...
          {activeTab === 'pendingEnrollments' && (
            <>
              <h3 className="text-xl font-semibold mb-4">Pending Enrollments</h3>
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 mb-4">
                <button
                  className="btn btn-secondary"
                  onClick={exportPendingEnrollmentsToCSV}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border p-2">Access Requested</th>
                ...
                <tbody>
                  {filtered(pendingEnrollments).map((e) => (
                    <tr key={`${e.userId}-${e.courseId}`} className="hover:bg-gray-50">
                      <td className="border p-2">{e.user?.name}</td>
                      <td className="border p-2">{e.user?.email}</td>
          ...
          {activeTab === 'approvedEnrollments' && (
            <>
              <h3 className="text-xl font-semibold mb-4">Approved Enrollments</h3>
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 mb-4">
                <button
                  className="btn btn-secondary"
                  onClick={exportApprovedEnrollmentsToCSV}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border p-2">Email</th>
                    <th className="border p-2">Course</th>
                    <th className="border p-2">Access Granted</th>
                    <th className="border p-2">Status</th>
                  </tr>
                </thead>
                <tbody>
