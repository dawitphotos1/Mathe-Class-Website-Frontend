...
const AdminDashboard = () => {
  ...
  const enrollmentStats = {
    ...
    approved: approvedEnrollments.length,
    rejected: rejectedEnrollments.length,
    total: pendingEnrollments.length + approvedEnrollments.length + rejectedEnrollments.length,
  };
  ...
  // ✅ FIXED: Approve Student with better error handling
  const handleApproveStudent = async (id) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.error || "Failed to approve student");
      }
    } catch (err) {
  ...
  const studentList = activeStudentTab === "pending" ? pendingStudents :
                     activeStudentTab === "approved" ? approvedStudents : rejectedStudents;

  const enrollList = activeEnrollTab === "pending" ? pendingEnrollments :
                    activeEnrollTab === "approved" ? approvedEnrollments : rejectedEnrollments;
  ...
  return (
    <div className={`dashboard-container ${isDark ? "dark-mode" : ""}`}>
      ...
      <div className="admin-section">
        <h3>Student Account Approvals</h3>
        <div className="section-description">Manage student account registration requests</div>
        ...
        <div className="admin-tabs">
          {["pending", "approved", "rejected"].map((tab) => (
            ...
            >
              {tab.charAt(0).toUpperCase() + tab.slice(1)} (
              {tab === "pending" ? studentStats.pending :
               tab === "approved" ? studentStats.approved : studentStats.rejected})
            </button>
          ))}
        </div>
          ...
          <div className="table-container">
            <table className="admin-table">
              ...
              <tbody>
                {studentList.length === 0 ? (
                  <tr>
                    <td colSpan={activeStudentTab === "pending" ? 6 : 5} className="no-data">
                      No {activeStudentTab} students found
                    </td>
                  </tr>
                ) : (
                  studentList.map((student) => (
                    <tr key={student.id} className={student.approval_status === "rejected" ? "rejected-row" : ""}>
                      <td>{student.name}</td>
                      <td>{student.email}</td>
                      <td>{student.subject || "N/A"}</td>
                      <td>
                      ...
                      <td>{new Date(student.updatedAt || student.createdAt).toLocaleDateString()}</td>
                      {activeStudentTab === "pending" && (
                        <td className="action-buttons">
      ...
      <div className="admin-section">
        <h3>Course Enrollment Approvals</h3>
        <div className="section-description">Manage course enrollment requests after payment</div>
        ...
        <div className="admin-tabs">
          {["pending", "approved", "rejected"].map((tab) => (
            ...
            >
              {tab.charAt(0).toUpperCase() + tab.slice(1)} (
              {tab === "pending" ? enrollmentStats.pending :
               tab === "approved" ? enrollmentStats.approved : enrollmentStats.rejected})
            </button>
          ))}
        </div>
          ...
          <div className="table-container">
            <table className="admin-table">
              ...
              <tbody>
                {enrollList.length === 0 ? (
                  <tr>
                    <td colSpan={activeEnrollTab === "pending" ? 7 : 6} className="no-data">
                      No {activeEnrollTab} enrollments found
                    </td>
                  </tr>
                ) : (
                  enrollList.map((enrollment) => (
                    <tr key={enrollment.id} className={enrollment.approval_status === "rejected" ? "rejected-row" : ""}>
                      <td>{enrollment.student?.name || "N/A"}</td>
                      <td>{enrollment.student?.email || "N/A"}</td>
                      <td>{enrollment.course?.title || "N/A"}</td>
                      <td>
