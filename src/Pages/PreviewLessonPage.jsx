...
const PreviewLessonPage = () => {
  ...
  const fetchPreviewLesson = async () => {
    try {
      ...
      if (lessonId) {
        ...
        const courseResponse = await axiosInstance.get(`/courses/slug/${slug}`);
        if (courseResponse.data.success) {
          apiUrl = `/courses/${courseResponse.data.course.id}/preview-lesson`;
        }
      }
      ...
      setError(err.message || "Failed to load preview content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBack = () => {
    navigate(courseId || slug ? `/courses/${courseId || slug}` : "/courses");
  };

  const handleDownload = () => {
    if (lesson?.fileUrl) window.open(lesson.fileUrl, "_blank");
  };

  const handleViewFile = () => {
    if (lesson?.fileUrl) window.open(lesson.fileUrl, "_blank", "noopener,noreferrer");
  };
  ...
  const renderContent = () => {
    ...
    if (lesson.fileUrl && lesson.fileUrl.includes(".pdf")) {
      return (
        <Box className="preview-content">
          <Button startIcon={<Visibility />} onClick={handleViewFile}>
            View PDF
          </Button>
    ...
    if (lesson.textContent) {
      return (
        <Paper elevation={0}>
          <Typography>{lesson.textContent}</Typography>
        </Paper>
      );
    }
      ...
      <Card>
        <CardContent>
          <Box className="preview-header">
            ...
            <Chip icon={<Visibility />} label="Preview Mode" />
            {user?.role === "teacher" && (
              <Chip icon={<School />} label="Teacher View" />
            )}
          </Box>
