...
//   const [viewerLessonId, setViewerLessonId] = useState(null);
...
//   const openViewerForLesson = (lessonId) => {
//     setViewerLessonId(lessonId);
//     setViewerOpen(true);
//   };
...
//                   <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
//                     <Button size="small" startIcon={<EditIcon />} onClick={() => handleEditCourse(course.id)} variant="outlined">
//                       Edit Course
//                     </Button>
...
//       <FullscreenLessonViewer open={viewerOpen} onClose={closeViewer} lessonId={viewerLessonId} darkMode={isDark} />
...
//             <strong>{deleteDialog.type === "course" ? deleteDialog.course?.title : deleteDialog.lesson?.title}</strong>?
//           </Typography>
...
/**
 * MyTeachingCourses.jsx - Updated to use LessonPreview (fullscreen)
 * - Keeps previous features: create/edit/delete/manage
 * - Robust compatibility for snake_case/camelCase lesson objects
 */
...
const MyTeachingCourses = () => {
  ...
  // preview state using LessonPreview component
  ...
  const fetchTeacherCourses = async () => {
    try {
      ...
      if (coursesResponse.data?.success) {
        ...
        // fetch full structure for each course (units+lessons) in parallel
        const coursesWithLessons = await Promise.all(
          coursesData.map(async (course) => {
            try {
              const structureResponse = await axiosInstance.get(
  ...
  // open preview using LessonPreview dialog
  const handlePreviewLesson = (lesson, course) => {
    // Accept lesson shapes in different formats (normalize lightly)
    const normalizedLesson = {
      ...lesson,
      // unify commonly used fields
      fileUrl: lesson.fileUrl ?? lesson.file_url ?? lesson.file ?? null,
      videoUrl: lesson.videoUrl ?? lesson.video_url ?? lesson.video ?? null,
      contentType: lesson.contentType ?? lesson.content_type ?? lesson.type ?? null,
      textContent: lesson.textContent ?? lesson.content ?? lesson.text ?? "",
      title: lesson.title ?? lesson.name ?? "Lesson",
      isPreview: lesson.is_preview ?? lesson.isPreview ?? false,
    };

    setPreviewState({ open: true, lesson: normalizedLesson, course });
  };

  const handleClosePreview = () => {
    setPreviewState({ open: false, lesson: null, course: null });
  };
