...
const MyTeachingCourses = () => {
  ...
  const fetchTeacherCourses = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/courses/teacher/my-courses");
      ...
      if (res.data.success) {
        ...
        for (const course of res.data.courses) {
          const structure = await axiosInstance.get(`/courses/teacher/${course.id}/full`);
          lessonsMap[course.id] = structure.data?.course?.units?.flatMap(
            (unit) => unit.lessons || []
          );
        }
          ...
          <Grid item xs={12} key={course.id}>
            <Card>
              <CardContent>
                ...
                <Chip label={`${lessons[course.id]?.length || 0} Lessons`} />

                {/* Accordion Lessons */}
                <Accordion
                  expanded={expandedCourse === course.id}
                  onChange={() => setExpandedCourse(course.id)}
                  sx={{ mt: 2 }}
                >
                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                        ...
                        {lessons[course.id].map((lesson) => (
                          <ListItem key={lesson.id}>
                            ...
                            <Button
                              ...
                              onClick={() => navigate(`/lessons/${lesson.id}/preview`)}
                              sx={{ mr: 1 }}
                            >
                              Preview
                            </Button>
                            ...
                            <ListItemSecondaryAction>
                              <IconButton onClick={() => navigate(`/lessons/${lesson.id}/edit`)}>
                                <EditIcon />
                              </IconButton>
              ...
              <CardActions>
                <Button startIcon={<AddIcon />} onClick={() => navigate(`/courses/${course.id}/lessons/new`)}>
                  Create Lesson
                </Button>
                <Button startIcon={<EditIcon />} onClick={() => navigate(`/courses/${course.id}/edit`)}>
                  Edit Course
                </Button>
              </CardActions>
            </Card>
