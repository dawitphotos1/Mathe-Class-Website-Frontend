...
const MyTeachingCourses = () => {
    ...
    try {
      ...
      if (coursesResponse.data?.success) {
        ...
        const coursesWithLessons = await Promise.all(
          coursesData.map(async (course) => {
            try {
              const structureResponse = await axiosInstance.get(`/courses/teacher/${course.id}/full`);
              return {
                courseId: course.id,
                lessons:
                  structureResponse.data?.course?.units?.flatMap((unit) => unit.lessons || []) || [],
              };
            } catch (error) {
  ...
  const handlePreviewLesson = async (lesson) => {
    try {
      if (!lesson?.id) {
        showSnackbar("Invalid lesson", "error");
        return;
      }
      // Use the public unified endpoint to find preview lesson (consistent)
      // If teacher already has lesson.id (which they do), we can navigate directly:
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      console.error(`Error deleting ${type}:`, error);
      const errorMessage =
        error.response?.data?.message || error.message || `Failed to delete ${type}`;
      showSnackbar(errorMessage, "error");
    } finally {
      setDeleting(false);
      setDeleteDialog({ open: false, course: null, lesson: null, type: null });
  ...
  return (
    <Box sx={{ p: 3 }} className={`teacher-dashboard ${isDark ? "dark-mode" : ""}`}>
      <Typography variant="h4" gutterBottom sx={{ mb: 3 }}>
        📘 My Teaching Courses
      </Typography>
      ...
      <Card className="creation-options-card">
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <Card variant="outlined" className="creation-option simple" sx={{ p: 2, height: "100%", cursor: "pointer" }} onClick={() => navigate("/create-course")}>
              <Box className="creation-option-header">
                <RocketIcon className="floating" />
                <Typography variant="h6" gutterBottom>
                  Simple Course Creation
          ...
          <Grid item xs={12} md={6}>
            <Card variant="outlined" className="creation-option advanced" sx={{ p: 2, height: "100%", cursor: "pointer" }} onClick={() => navigate("/create-course-advanced")}>
              <Box className="creation-option-header">
                <BuildIcon className="floating" />
                <Typography variant="h6" gutterBottom>
                  Advanced Course Creation
              ...
              <Card className="course-card">
                <CardContent>
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2 }}>
                    <Box>
                      <Typography variant="h5" component="h2" gutterBottom>{course.title}</Typography>
                      <Typography variant="body1" color="textSecondary" paragraph>{course.description || "No description available."}</Typography>
                    </Box>
                  ...
                  <Accordion expanded={expandedCourse === course.id} onChange={() => handleCourseClick(course.id)} className="lessons-accordion">
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                      <Typography variant="h6">📦 Course Content ({lessons[course.id]?.length || 0} Lessons)</Typography>
                    </AccordionSummary>
                    <AccordionDetails>
                      {lessons[course.id]?.length > 0 ? (
                        <List dense className="lessons-list">
                          {lessons[course.id].map((lesson) => (
                            <ListItem key={lesson.id} className="lesson-item main-lesson">
                                ...
                                <ListItemText
                                  primary={
                                    <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                                      ...
                                      <Button size="small" className="preview-button" startIcon={<PreviewIcon />} onClick={(e) => { e.stopPropagation(); handlePreviewLesson(lesson); }} sx={{ ml: 1 }}>
                                        Preview Lesson
                                      </Button>
                                      {lesson.is_preview && <Chip label="Free Preview" size="small" color="success" variant="filled" />}
                                    </Box>
                                  }
                                  secondary={
                                    <Box sx={{ display: "flex", gap: 1, mt: 0.5, flexWrap: "wrap" }}>
                                      <Chip label={getContentTypeLabel(lesson.content_type)} size="small" variant="outlined" />
                              ...
                              <ListItemSecondaryAction>
                                <IconButton edge="end" aria-label="edit" onClick={() => handleEditLesson(course.id, lesson.id)} size="small" sx={{ mr: 1 }}>
                                  <EditIcon />
                                </IconButton>
                                <IconButton edge="end" aria-label="delete" onClick={() => handleDeleteLesson(lesson, course.id)} size="small" color="error">
                                  <DeleteIcon />
                                </IconButton>
                              </ListItemSecondaryAction>
                            </ListItem>
                        ...
                        <Typography variant="body2" color="textSecondary" sx={{ textAlign: "center", py: 2 }}>No lessons yet. Create your first lesson!</Typography>
                      )}
                    </AccordionDetails>
                  </Accordion>
                </CardContent>
                ...
                <CardActions className="course-actions">
                  <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                    <Button size="small" startIcon={<SettingsIcon />} onClick={() => handleManageLessons(course.id)} variant="outlined">Manage Lessons</Button>
                    <Button size="small" startIcon={<AddIcon />} onClick={() => handleCreateLesson(course.id)} variant="outlined">Create Lesson</Button>
                  </Box>
                  <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                    <Button size="small" startIcon={<EditIcon />} onClick={() => handleEditCourse(course.id)} variant="outlined">Edit Course</Button>
                    <Button size="small" startIcon={<DeleteIcon />} onClick={() => handleDeleteCourse(course)} variant="outlined" color="error" disabled={deleting}>
                      {deleting ? "Deleting..." : "Delete"}
                    </Button>
                  </Box>
                </CardActions>
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, course: null, lesson: null, type: null })}>
        <DialogTitle>Delete {deleteDialog.type === "course" ? "Course" : "Lesson"}</DialogTitle>
        <DialogContent>
          <Typography>
            Are you sure you want to delete <strong>{deleteDialog.type === "course" ? deleteDialog.course?.title : deleteDialog.lesson?.title}</strong>?
          </Typography>
          <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
            {deleteDialog.type === "course" ? "This will also delete all lessons in this course. This action cannot be undone." : "This action cannot be undone."}
          </Typography>
          {deleteDialog.type === "course" && <Alert severity="warning" sx={{ mt: 2 }}>⚠️ Warning: This will permanently delete the course and all its content!</Alert>}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteDialog({ open: false, course: null, lesson: null, type: null })} disabled={deleting}>Cancel</Button>
          <Button onClick={confirmDelete} color="error" variant="contained" disabled={deleting} startIcon={deleting ? <CircularProgress size={16} /> : <DeleteIcon />}>
            {deleting ? "Deleting..." : "Delete"}
          </Button>
        </DialogActions>
      </Dialog>
