...
const MyTeachingCourses = () => {
  ...
  const fetchCourses = async () => {
    try {
      const res = await api.get("/api/v1/courses/teacher");

      const coursesWithLessons = await Promise.all(
        res.data.courses.map(async (course) => {
          const lessonsRes = await api.get(
            `/api/v1/lessons/course/${course.id}/lessons`
          );
          const units = lessonsRes.data.units || [];
          return { ...course, units };
        })
      );
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/api/v1/courses/${courseToDelete.id}`);
      toast.success("Course deleted");
        ...
        courses.map((course) => (
          <div key={course.id} className="course-card">
            ...
            <div className="course-buttons">
              <Link to={`/course/${course.id}/create-lesson`} className="btn">
                ➕ Create Lesson
              </Link>
              <Link to={`/course/${course.id}/manage-lessons`} className="btn">
                🛠 Manage Lessons
              </Link>
              <Link to={`/edit-course/${course.id}`} className="btn">
                ✏️ Edit Course
              </Link>
              <button
                className="btn delete"
            ...
            <div className="lessons-section">
              <h4>📚 Created Lessons:</h4>
              {course.units.length === 0 ? (
                <p>No lessons yet.</p>
              ) : (
                course.units.map((unit) => (
                  <div key={unit.id || unit.unitName} className="unit-block">
                    <h5>{unit.unitName}</h5>
                    <ul>
                      {unit.lessons.map((lesson) => (
