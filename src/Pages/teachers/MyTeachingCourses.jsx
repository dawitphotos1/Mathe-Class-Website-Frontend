...
const MyTeachingCourses = () => {
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      if (res.data.success && Array.isArray(res.data.courses)) {
        ...
        // Auto-expand all units by default
        const expandedMap = {};
        Object.keys(lessonsMap).forEach((courseId) => {
          lessonsMap[courseId].forEach((unit) => {
            expandedMap[`${courseId}-${unit.unitName}`] = true;
  ...
  const deleteAttachment = (courseId, index) => {
    setModal({
      show: true,
      title: "Delete Attachment",
      message: "Are you sure you want to delete this attachment?",
      onConfirm: async () => {
        try {
          const res = await axiosInstance.patch(`/courses/${courseId}`, {
            removeAttachmentIndex: index,
          });
          toast.success("✅ Attachment deleted");
  ...
  return (
    <div className={`my-teaching-courses ${darkMode ? "dark" : ""}`}>
      ...
      {/* PDF Preview Modal */}
      {pdfPreview && (
        <div className="pdf-preview-modal" onClick={handleClosePdfPreview}>
          <div
            className="pdf-preview-content"
            onClick={(e) => e.stopPropagation()}
          >
            <button className="close-btn" onClick={handleClosePdfPreview}>
              ✖️
            </button>
            <iframe
              src={pdfPreview}
