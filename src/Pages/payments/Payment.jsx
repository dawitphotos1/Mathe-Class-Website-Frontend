  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        setLoading(true);
        setDebugInfo(
          `Fetching course ${courseId} from: ${axiosInstance.defaults.baseURL}`
        );
        ...
        if (response.data.success) {
          setCourse(response.data.course);
          setDebugInfo(
            `Course loaded: ${response.data.course.title} - Price: $${response.data.course.price}`
          );

          // Debug price value
          console.log("💰 Course price details:", {
            rawPrice: response.data.course.price,
            parsedPrice: parseFloat(response.data.course.price),
            type: typeof response.data.course.price,
          });
        } else {
          ...
          <div className="price-section">
            ...
            {!isValidPrice && (
              <div className="price-warning">
                <p>
                  ⚠️ Price appears to be $0. This might be a configuration
                  issue.
                </p>
