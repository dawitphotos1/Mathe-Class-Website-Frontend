...
//           `/api/v1/courses/${courseData.id}/lessons`,
//           { headers }
//         );
...
const StartCoursePage = () => {
  ...
  useEffect(() => {
    const fetchCourseAndLessons = async () => {
      try {
        ...
        // ✅ Public course fetch (NO /api/v1 prefix)
        const courseRes = await axios.get(`/courses/public/slug/${slug}`);
        const courseData = courseRes.data;
        ...
        // ✅ Fetch lessons + progress only if logged in
        if (token) {
          const lessonRes = await axios.get(
            `/courses/${courseData.id}/lessons`,
            { headers }
          );
          setLessons(lessonRes.data.lessons || []);
  ...
  const markComplete = async (lessonId) => {
    try {
      await axios.post(`/progress/complete`, { lessonId });
      setCompletedLessons((prev) => [...new Set([...prev, lessonId])]);
      toast.success("✅ Lesson marked complete!");
    } catch {
      toast.error("❌ Failed to update progress");
  ...
  return (
    <div className="course-container">
      ...
      {!localStorage.getItem("token") && (
        <p className="login-hint">🔑 Log in to view lessons and track progress.</p>
      )}
    </div>
  );
};
