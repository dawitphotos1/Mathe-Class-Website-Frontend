// // FULLY UPDATED MyCourses.jsx to support lessons rendering
...
const MyCourses = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      setLoading(true);
      try {
        // No need to manually set Authorization header if axiosInstance does it
  ...
  const fetchCourseLessons = async (courseId) => {
    ...
    try {
      const res = await axiosInstance.get(`/api/v1/courses/${courseId}/lessons`);
      if (res.data?.lessons) {
  ...
  const handleUnenrollConfirmed = async () => {
    if (confirmUnenroll === null) return;
    try {
      await axiosInstance.delete(`/api/v1/enrollments/${confirmUnenroll}`);
