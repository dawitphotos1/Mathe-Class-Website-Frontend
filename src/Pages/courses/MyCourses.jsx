...
const MyCourses = () => {
      ...
      try {
        ...
        const transformed = res.data.courses.map((course) => {
          ...
          return {
            ...
            category: course.category || getCategoryFromTitle(course.title),
        ...
        const errorMsg = err.response?.data?.error || "Failed to load courses";
  ...
  const handleUnenrollConfirmed = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.error || "Unenrollment failed. Try again."
      );
    } finally {
      setConfirmUnenroll(null);
    }
        ...
        <div className="no-courses">
          ...
          {tab === "approved" ? (
            ...
            <p className="pending-help">
              Your pending enrollments will appear here once submitted
            </p>
          )}
        </div>
      )}
