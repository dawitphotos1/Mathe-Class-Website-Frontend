...
// src/components/LessonPreview.jsx - FIXED VERSION (no new tab for PDFs)
...
const LessonPreview = ({ open, onClose, lesson }) => {
  ...
  // FIXED: Update iframe instead of opening new tab for PDFs
  const handleOpenFile = () => {
    if (!debugInfo?.normalizedFileUrl) return;
    
    // For PDFs, update the iframe src
    if (debugInfo.contentType.toLowerCase() === 'pdf' || debugInfo.contentType.toLowerCase() === 'file') {
      if (pdfIframeRef.current) {
        pdfIframeRef.current.src = debugInfo.normalizedFileUrl;
        console.log('🔄 PDF iframe URL updated in same window');
        setError(null);
      }
    } else {
      // For non-PDF, open in new tab
      window.open(debugInfo.normalizedFileUrl, "_blank", "noopener,noreferrer");
    }
  };
  ...
  const hasFile = !!debugInfo?.normalizedFileUrl;
  const isPdf = debugInfo?.contentType?.toLowerCase() === 'pdf' || debugInfo?.contentType?.toLowerCase() === 'file';
        ...
        <Toolbar>
          ...
          {debugInfo?.normalizedVideoUrl && (
            <Button
              color="inherit"
              startIcon={<PlayCircleOutlineIcon />}
              onClick={() => window.open(debugInfo.normalizedVideoUrl, "_blank")}
              sx={{ mr: 1 }}
            >
              Open Video
            </Button>
          ...
          {hasFile && (
            <>
              {/* FIXED: For PDFs, this updates iframe; for others, opens new tab */}
              <Button
                color="inherit"
                startIcon={<OpenInNewIcon />}
